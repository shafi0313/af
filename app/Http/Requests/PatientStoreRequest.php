...
class PatientStoreRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'f_name_en'    => 'required|string|max:191',
            'm_name'       => 'required|string|max:191',
            'm_name_en'    => 'required|string|max:191',
            'phone'        => 'required|string|max:30',
            'phone_en'     => 'required|string|max:30',
            'e_phone'      => 'required|string|max:30',
            'e_phone_en'   => 'required|string|max:30',
            'division_id'  => 'required',
            'district_id'  => 'required',
            'upazila_id'   => 'required',
            'post'         => 'required|string|max:191',
            'post_en'      => 'required|string|max:191',
            'village'      => 'required|string|max:191',
            'village_en'   => 'required|string|max:191',
            'finance'      => 'required|string|max:500',
            'finance_en'   => 'required|string|max:500',
            'hospital'     => 'required|string|max:191',
            'hospital_en'  => 'required|string|max:191',
            'doctor'       => 'required|string|max:191',
            'doctor_en'    => 'required|string|max:191',
