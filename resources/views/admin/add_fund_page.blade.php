@extends('admin.layout.app')

@section('content')

    @include('admin.inc.sidebar')
    <div class="se-pre-con"></div>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.12/summernote-lite.css" rel="stylesheet">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.12/summernote-lite.js"></script>

    <main class="main-content col-lg-10 col-md-9 col-sm-12 p-0 offset-lg-2 offset-md-3">
        <!-- / .main-navbar -->
        <div class="main-content-container container-fluid px-4">
            <!-- Page Header -->
            <div class="page-header row no-gutters py-4">
                <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                    <h3 class="page-title">Content Manage</h3>
                </div>
            </div>
            <div class="collapse my-4" id="collapseExample">
                <div class="card">
                    <div class="card-header border-bottom inputheader">Add</div>
                    <div class="card-body">
                        <form class="form-inline">
                             <input type="hidden" name="inputUnitId" id="inputUnitId" >
                            <div class="form-group mx-sm-3 mb-2">
                                <label for="inputCategory" class="sr-only">Type</label>
                                <select class="form-control" name="type" id="type" required="required">
                                    <option value="">Select Type</option>
                                    <option value="1">About Us</option>
                                    <option value="2">Terms & Conditions</option>
                                </select>
                            </div>

                            <div class="form-group mx-sm-3 mb-2">
                                <label for="inputCategory" class="sr-only">Title</label>
                                <input type="text" class="form-control" name="unitName" id="unitName" placeholder="Enter Title">
                               
                            </div>
                            <div class="form-group mx-sm-3 mb-2">
                                <label for="inputCategory" class="sr-only">Details</label>
                                <textarea class="emptydetails form-control" name="long_details" id="summernote" placeholder="Description"></textarea>
                            </div>
                            <div class="inputButton">
                                <button type="button" class="btn btn-primary mb-2 UpdateCategory">Update</button>
                            </div>
                            <button type="button" class="btn btn-success mx-3 mb-2 closer">Close</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <div class="card card-small mb-4">
                        <div class="card-header border-bottom">
                        </div>
                        <div class="card-body p-0 text-center ReactTable">
                            <table class="table mb-0 rt-table">
                                <thead class="bg-light">
                                <tr>
                                    <th scope="col" class="border-0">No</th>
                                    <th scope="col" class="border-0">Type</th> 
                                    <th scope="col" class="border-0">Title</th> 
                               
                                    <th scope="col" class="border-0">Action</th>
                                </tr>
                                </thead>                              
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>

    <script>
          $('#summernote').summernote({
        placeholder: 'Enter Details.....',
        tabsize: 2,
        height: 100
      });

        var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        $(document).ready(function () {
            $(function () {
                table.ajax.reload();
            });
            $(document).on('click', '.add', function () {
                $('.inputheader').html('Add Content');
                $('#unitName').val('');
                $('#inputUnitId').val('');
                $('.inputButton').html('<div id="inputButton"><button type="button" class="btn btn-primary mb-2 addUnit">Save</button></div>\n');
                $('.collapse').collapse('show');
            });
            $(document).on('click', '.closer', function () {
                $('.collapse').collapse('hide');
            });
            $(document).on('click', '.addUnit', function () {
                let type = $('#type').val();              
                let unitName = $('#unitName').val();              
                let detail = $('#summernote').val();               
                let inputUnitId = $('#inputUnitId').val();               
                if (type != '') {
                    if(unitName == ''){
                         Swal.fire('Input field empty')
                    }
                    if(detail == ''){
                         Swal.fire('Input field empty') 
                    }
                    $('#AddSubcategory').prop('disabled',true);
                    $.ajax({
                        url: '{{ route('admin.fund_store') }}',
                        type: 'post',
                        data: {_token: CSRF_TOKEN, unitName: unitName, type: type, detail: detail, inputUnitId: inputUnitId},
                        success: function (response) {

                            if (response == 1) {
                                const Toast = Swal.mixin({
                                    toast: true,
                                    position: 'top-end',
                                    showConfirmButton: false,
                                    timer: 3000
                                });

                                Toast.fire({
                                    type: 'success',
                                    title: 'Content Inserted successfully'
                                })
                                table.ajax.reload();
                                $('#AddSubcategory').prop('disabled',false);
                                $('#subcategory').val('');
                            } else if (response == 2) {
                               const Toast = Swal.mixin({
                                    toast: true,
                                    position: 'top-end',
                                    showConfirmButton: false,
                                    timer: 3000
                                });

                                Toast.fire({
                                    type: 'success',
                                    title: 'Content Updated successfully'
                                })
                                table.ajax.reload();
                                $('#AddSubcategory').prop('disabled',false);
                                $('#subcategory').val('');
                            }
                            $('#summernote').val('');
                            $('#unitName').val('');
                            $('#inputUnitId').val('');  
                            $('.collapse').collapse('hide');
                        }
                    });
                } else {
                    Swal.fire('Input field empty')
                }
            });
            let table = $('.table').DataTable({
                processing: true,
                serverSide: true,                                 
                ajax: "{{ url('view-unit') }}",
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},                   
                    {data: 'type'},   
                    {data: 'title'},               
                    
                    {data: 'action'},           
                ],
                
                /*"footerCallback": function (tfoot, data, start, end, display) {
                    var api = this.api();
                    var p = api.column(2).data().reduce(function (a, b) {
                        return parseInt(a) + parseInt(b);
                    }, 0)
                    $(api.column(2).footer()).html("Total: "+p);
                 }*/

            });

            $(document).on('click', '.delete', function () {
                let id = $(this).attr('id');
                Swal.fire({
                    title: 'Are you sure?',
                    text: "You won't be able to revert this!",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Yes, delete it!',
                    animation: false,
                    customClass: 'animated bounceInDown'
                }).then((result) => {
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('delete-unit') }}",
                            type: 'get',
                            data: {id: id,},
                            success: function (response) {
                                if (response == 1) {
                                    table.ajax.reload();
                                    Swal.fire({
                                        type: 'success',
                                        title: 'Deleted',
                                        text: 'This Unit has been deleted.',
                                        animation: false,
                                        customClass: 'animated tada'
                                    })
                                } else {
                                    Swal.fire(
                                        'Deleted!',
                                        'Something Wrong',
                                        'error'
                                    )
                                }

                            }
                        });
                    }
                })
            });
            $(document).on('click', '.edit', function () {
                $('.collapse').collapse('show');
                let id = $(this).attr('id');
                $.ajax({
                    url: "{{ url('admin/view-edit-unit') }}",
                    type: 'get',
                    data: {id: id,},
                    dataType: 'json',
                    success: function (data) {
                           console.log(data.title);
                        $('.inputheader').html('Update Unit Name');                     
                        $('#inputUnitId').val(data.id);
                        $('#type').val(data.type);
                        $('#unitName').val(data.title);
                        $('#summernote').summernote('code', data.details);
                        $('.inputButton').html('<div id="inputButton"><button type="button" class="btn btn-primary mb-2 addUnit">Update</button></div>\n');
                    }
                });
            });


            $(document).on('click', '.UpdateUnit', function () {
                event.preventDefault();
                $('.collapse').collapse('show');
                let name = $('#unitName').val(),
                    id = $('#inputUnitId').val();
                if (name != '') {
                    $('#AddSubcategory').prop('disabled',true);
                    $.ajax({
                        url: "{{ url('update-unit') }}",
                        type: 'post',
                        data: {_token: CSRF_TOKEN, name: name, id: id},
                        success: function (data) {
                            if (data == 1) {
                                const Toast = Swal.mixin({
                                    toast: true,
                                    position: 'top-end',
                                    showConfirmButton: false,
                                    timer: 3000
                                });

                                Toast.fire({
                                    type: 'success',
                                    title: 'Unit Name Update successfully'
                                })
                                $('#AddSubcategory').prop('disabled',false);
                                $('.collapse').collapse('hide');
                                table.ajax.reload();
                            } else {
                                Swal.fire(
                                    response,
                                    'Something wrong, please contact administrator.',
                                    'error'
                                )
                                $('#AddSubcategory').prop('disabled',false);
                            }
                        }
                    });
                } else {
                    Swal.fire('Input field empty')
                    $('#AddSubcategory').prop('disabled',false);
                }
            });

        });
    </script>

@endsection
